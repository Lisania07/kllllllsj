...
void shop() {
  ...
  text("50 coins", width/2-175, height/2+75);
  text("lives+3", width/2-25, height/2+50);
  text("80 coins", width/2-25, height/2+75);
  text("lives+5", width/2+125, height/2+50);
  text("100 coins", width/2+125, height/2+75);
}
...
void shopClicks() {
 println(player1.lives);
  if (mouseX>width/2-175 && mouseX<width/2-125 && mouseY>height/2-25 && mouseY<height/2+25 &&bank>=50) {
   live=2;
    player1.lives=player1.lives+2;
    bank=bank-50;
  }
  if (mouseX>width/2-25 && mouseX<width/2+25 && mouseY>height/2-25 && mouseY<height/2+25 && bank>=80) {
    live=3;
    player1.lives=player1.lives+3;
    bank=bank-80;
  }
  if (mouseX>width/2+125 && mouseX<width/2+175 && mouseY>height/2-25 && mouseY<height/2+25 && bank>=100) {
    live=5;
